...
class ChangeWatcher(Thread):
    ...
    def run(self):
        ...
        indicator_action = dao.read_all_pe_data(
            "NIFTY 50")  # pd.read_csv(sys.path[0] + "\\NIFTY 50_pe.csv", delimiter=",")
        merge_pd = price_action.merge(indicator_action, how="left", on="Date")
        # print(merge_pd.dtypes)
        # print(merge_pd["P_E"].describe())
        merge_pd.sort_values("P_E", ascending=True, inplace=True)
        merge_pd["P_E_Range"] = pd.cut(merge_pd["P_E"],
                                       bins=(int(merge_pd["P_E"].max()) - int(merge_pd["P_E"].min() + 1)), precision=2,
                                       ordered=True)

        new_pd = merge_pd["P_E_Range"].value_counts() / merge_pd["P_E"].count()
        ...
        last_pe = str(merge_pd["P_E"].iloc[-1])
